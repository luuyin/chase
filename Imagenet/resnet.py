...
class Bottleneck(nn.Module):
    ...
    def __init__(self, builder, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = builder.conv1x1(inplanes, wider_planes)
        self.bn1 = builder.batchnorm(wider_planes)
        self.conv2 = builder.conv3x3(wider_planes, wider_planes, stride=stride)
        self.bn2 = builder.batchnorm(wider_planes)
        self.conv3 = builder.conv1x1(wider_planes, planes * self.expansion)
...
class ResNet(nn.Module):
    def __init__(self, builder, block, layers, num_classes=1000):
        ...
        for block_ind in range(len(layers)):
    
            for layer_index in range(layers[block_ind]):
        ...
        #             next_layers[("layer"+str(block_ind),layer_index,"conv3")]=[["layer"+str(block_index),layer_index,"shortcut"],["layer"+str(block_index),layer_index,"BatchNorm2d"]]
                ...
                self.next_layers[("module","layer"+str(block_index),layer_index,"conv1")]=[["module","layer"+str(block_index),layer_index,"conv2"],["module","layer"+str(block_index),layer_index,"bn1"]]
                self.next_layers[("module","layer"+str(block_index),layer_index,"conv2")]=[["module","layer"+str(block_index),layer_index,"conv3"],["module","layer"+str(block_index),layer_index,"bn2"]]
