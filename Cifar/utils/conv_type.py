...
class STRConv(nn.Conv2d):
    ...
    def getSparsity(self,name="layer", f=torch.sigmoid):
        sparseWeight = sparseFunction(self.weight, self.sparseThreshold,  self.activation, self.f)
        temp = sparseWeight.detach().cpu()

        for channel_vector in temp:
            channel_zero=(channel_vector!=0).sum().int().item()
            channel_all=channel_vector.numel()

            # print("check in", name, "density is",channel_zero/channel_all,"weight magnitue", torch.abs(channel_vector).mean().item() , channel_vector.shape)
